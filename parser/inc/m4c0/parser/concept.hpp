...
namespace m4c0::parser {
  ...
  template<typename Fn, typename P>
  concept accepts_directly_nothrow =
      accepts_directly<Fn, P> && std::is_nothrow_invocable_v<Fn, typename std::invoke_result_t<P, input_t>::type>;

  template<typename Fn, typename P>
  concept accepts_with_remainder = std::is_invocable_v<Fn, typename std::invoke_result_t<P, input_t>::type, input_t>;
  template<typename Fn, typename P>
  concept accepts_with_remainder_nothrow = accepts_with_remainder<Fn, P> && std::
      is_nothrow_invocable_v<Fn, typename std::invoke_result_t<P, input_t>::type, input_t>;

  template<typename Fn, typename P>
  concept accepts = accepts_directly<Fn, P> || accepts_with_remainder<Fn, P>;
  template<typename Fn, typename P>
  concept accepts_nothrow = accepts_directly_nothrow<Fn, P> || accepts_with_remainder_nothrow<Fn, P>;
  ...
  template<typename P>
  concept is_nothrow_parser = is_parser<P> && noexcept(std::declval<P>()(input_t {}));
}
